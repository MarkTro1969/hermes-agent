...
class TestWorkerTeardownOnCeiling:
    ...
    def test_uninterruptible_worker_is_orphaned_with_lease_retained(self):
        ...
        def stuck_worker(fence: CompressionCommitFence):
            # Publish progress before the host starts enforcing the idle watchdog;
            # otherwise thread startup latency can turn this total-ceiling test into
            # an idle-timeout test under a saturated parallel suite.
            fence.touch_progress()
            worker_started.set()
        ...
        assert msgs is original, "poisoned late result must never be adopted"
